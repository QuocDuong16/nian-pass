...
## M5.0 shared mobile foundation

M5.0 keeps one application host and one Rust vault implementation. The
...
The Android project at `apps/desktop/src-tauri/gen/android` is generated by the
repository-pinned Tauri CLI. It compiles the existing Tauri Rust package, which
continues to depend on `vault-session` and `vault-core`; transitively the same
sealed `kdbx` adapter remains authoritative. Generated Kotlin is Tauri runtime
glue only. It does not parse KDBX, reconstruct vault models, persist credentials,
or implement a second save transaction.
...
Desktop targets register the existing dialog and clipboard integrations,
install `AppState`, and expose the reviewed desktop command surface. Android
and future iOS targets expose only `runtime_info` during M5.0. Their passive
foundation view therefore cannot mount desktop close/focus lifecycle, file
selection, clipboard behavior, unlock, mutation, save, or Lock. This separation
preserves all M4.5 desktop behavior while preventing desktop integration
semantics from becoming an accidental mobile contract.

```text
Rust application semantics
        ^
future Tauri mobile plugin bridge
     /          \
 Kotlin        Swift
(Android)      (iOS)
```

Kotlin and Swift may integrate operating-system services in M5.3/M5.4. They do
not become vault cores. Android document-provider content URIs are not canonical
file paths and are not accepted by the current M3 persistence contract.
M5.1/M5.2 must deliberately define picker handle ownership and a persistence
adapter without changing `VaultSession` canonical paths, source fingerprints,
verified replacement, backups, or external modification detection.

Android uses API 26 as its minimum because future AutofillService integration
requires Android 8.0; Autofill is not implemented in M5.0. The normal generated
...
package path. M5.0 does not create `apps/mobile/src-tauri`; mobile vault commands
remain disabled until later milestones supply the required native storage and
