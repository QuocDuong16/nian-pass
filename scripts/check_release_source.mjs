...
export function windowsRuntimeDiagnosticWorkflowViolations(root) {
  ...
  require(/cargo test --locked --target x86_64-pc-windows-msvc -p vault-session create_rejects_existing_target_without_deleting_its_contents/, "must prove Windows vault creation preserves an existing target");
  require(/cargo test --locked --target x86_64-pc-windows-msvc -p vault-session create_cleanup_preserves_external_replacement_at_published_target/, "must prove Windows Create cleanup preserves an external replacement");
  require(/cargo test --locked --target x86_64-pc-windows-msvc -p vault-session create_cleanup_does_not_recreate_a_target_that_disappeared/, "must prove Windows Create cleanup does not recreate a removed target");
  ...
  require(/cargo test --locked --target x86_64-pc-windows-msvc -p vault-session windows_candidate_create_edit_save_reopen_preserves_previous_generation/, "must prove the candidate Windows create/edit/save/reopen path and exact backup generations");
  require(/cargo test --locked --target x86_64-pc-windows-msvc -p nian-pass-desktop create_vault_picker_flow_rejects_unsupported_persistence_before_picker/, "must prove Windows Create Vault fails before opening its picker while persistence is disabled");
  ...
  require(/WINDOWS_REPLACEFILE_NATIVE_TESTS=PASS[\s\S]*?WINDOWS_REPLACEFILE_DACL_PRESERVATION=PASS[\s\S]*?WINDOWS_CREATE_EXISTING_TARGET_PRESERVED=PASS[\s\S]*?WINDOWS_CREATE_EXTERNAL_REPLACEMENT_PRESERVED=PASS[\s\S]*?WINDOWS_CREATE_DISAPPEARED_TARGET_NOT_RECREATED=PASS[\s\S]*?WINDOWS_REPLACEMENT_TRANSACTION_TESTS=PASS[\s\S]*?WINDOWS_ORDINARY_SAVE_CANDIDATE_PIPELINE=PASS[\s\S]*?WINDOWS_ORDINARY_SAVE_COVERED_CREATE_EDIT_SAVE_REOPEN=PASS[\s\S]*?WINDOWS_DESKTOP_CREATE_BLOCKED_BEFORE_PICKER=PASS[\s\S]*?WINDOWS_ORDINARY_SAVE_BACKUP_ROTATION=PASS[\s\S]*?WINDOWS_ORDINARY_SAVE_WRONG_CREDENTIAL=PASS[\s\S]*?WINDOWS_ORDINARY_SAVE_EXTERNAL_CONFLICT=PASS[\s\S]*?WINDOWS_ORDINARY_SAVE_COMPOSITE_CREDENTIALS=PASS[\s\S]*?WINDOWS_ORDINARY_SAVE_FAIL_CLOSED=PASS/, "must record separate Windows persistence evidence markers");
  if (
