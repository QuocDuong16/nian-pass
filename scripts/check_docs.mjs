...
export function runChecks(root) {
  ...
  requirePattern(violations, "README.md", readme, /M5\.5\s+Android Mobile Security \/ Lifecycle\s+DONE[\s\S]{0,240}M6\s+Browser Extension Foundation\s+DONE[\s\S]{0,240}M6\.5\s+Browser Native Messaging \/ Desktop Integration\s+DONE[\s\S]{0,160}M7\s+BYO-cloud Sync Providers\s+DONE[\s\S]{0,160}M7\.5\s+Self-hosted Sync Gateway\s+(?:NEXT|DONE)/i, "M7-done and M7.5 roadmap status is missing");
