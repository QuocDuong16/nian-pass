...
test("Linux native host ZIP preserves only the requested executable mode", () => {
  const archive = buildNativeHostArchive("linux-x86_64", Buffer.from("synthetic host"), "0.1.0");
  ...
  assert.deepEqual(archive, buildNativeHostArchive("linux-x86_64", Buffer.from("synthetic host"), "0.1.0"));
});

test("Windows native host ZIP keeps conservative Unix mode metadata", () => {
  const archive = buildNativeHostArchive("windows-x86_64", Buffer.from("synthetic host"), "0.1.0");
...
test(
  ...
  (t) => {
    ...
    writeFileSync(archivePath, buildNativeHostArchive("linux-x86_64", Buffer.from("synthetic host"), "0.1.0"));
